...
def rent_rig(rent_id, price, profile):
    ...
    params = {'rig': int(rent_id), 'length': float(3), 'profile': profile, 'currency': 'ETH',
              'rate.price': float(price)}
...
def get_rental_hash():
    ...
    try:
        for i in r['data']['rentals']:
            ...
            rental_spend += float((i['price']['paid']))
    finally:
        return rental_hash, rental_spend


#TODO return rental price and calculate avg
